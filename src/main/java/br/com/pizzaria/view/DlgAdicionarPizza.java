...
public class DlgAdicionarPizza extends JDialog {
	private static final int MAXIMO_ADICIONAIS = 3;
	...
	private class ActionAdicionarAdicionais implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			try {
				...
				if (adicionouMaximoSabores(pedidoPizza.getAdicionais())) {
					return;
				}
	...
	public boolean adicionouMaximoSabores(List<Adicional> adicionais) {
		
		if(adicionais.size() == MAXIMO_ADICIONAIS) {
			JOptionPane.showMessageDialog(botaoOk, "A pizza pode ter no máximo " + MAXIMO_ADICIONAIS + " adicionais.");
			return true;
		}
