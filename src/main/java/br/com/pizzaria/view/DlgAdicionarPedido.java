...
public class DlgAdicionarPedido extends JDialog {
	...
	private boolean testaClienteVazio(JComboBox<Cliente> field, String label) {

		Cliente cliente = (Cliente) field.getSelectedItem();
	...
	private boolean preencheuPizza(List<PedidoPizza> listaPedidoPizza) {

		if (listaPedidoPizza.size() > 0) {
			return true;
		}
	...
	private boolean preencheuBebida(List<PedidoBebida> listaPedidoBebida) {

		if (listaPedidoBebida.size() > 0) {
			return true;
		}
	...
	private class ActionOk implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (testaClienteVazio(comboCliente, "cliente")) {
				return;
			}
			...
			if (preencheuPizzaOuBebida(pedido.getPedidoPizzas(), pedido.getPedidoBebidas())) {
				return;
			}
	...
	public boolean preencheuPizzaOuBebida(List<PedidoPizza> pedidoPizzas, List<PedidoBebida> pedidoBebidas) {

		if (!preencheuPizza(pedidoPizzas) && !preencheuBebida(pedidoBebidas)) {
			JOptionPane.showMessageDialog(botaoOk, "Necessário adicionar pelo menos uma pizza ou uma bebiba.");
			return true;
		}
